...
/* === TAMBAHAN: PALET & LOGIKA DARK MODE === */
html[data-theme="dark"] {
  color-scheme: dark;
  --brand-blue: #6c95d8; /* Sedikit lebih cerah agar menonjol */
  --brand-blue-rgb: 108, 149, 216;
  --brand-cta: #EE4D2D;
  --success: #4cd8e9;
  --warning: #f8d68f;
  --error: #d86687;
  --dark-base: #e9ecf3; /* Kebalikan dari light mode */
...
/* === LOGIKA TAMPIL/SEMBUNYI IKON DARK MODE (PERBAIKAN) === */

/* 1. Sembunyikan KEDUA ikon secara default di dalam tombol */
...
@media (min-width: 769px) {
  ...
  .account-card-grid {
    grid-template-columns: repeat(2, 1fr); /* Aturan desktop: 2 kolom */
  }
  .affiliate-grid {
    grid-template-columns: repeat(3, 1fr); /* Aturan desktop: 3 kolom */
  }
}
...
/* === PERBAIKAN BAYANGAN KOTAK FOKUS === */
.icon-btn:focus-visible {
  box-shadow: none; /* Menghilangkan bayangan kotak biru global */
  background-color: var(--surface-secondary); /* Menggantinya dengan background halus (sama seperti hover) */
}
/* === AKHIR PERBAIKAN === */
...
.account-card-grid { display: grid; grid-template-columns: 1fr; gap: var(--sp-fib-5); margin-top: var(--sp-fib-5); } /* Aturan mobile: 1 kolom */
.affiliate-grid { display: grid; grid-template-columns: 1fr; gap: var(--sp-fib-5); margin-top: var(--sp-fib-5); } /* Aturan mobile: 1 kolom */
...
.library-grid {
  display: grid;
  grid-template-columns: repeat(3, 1fr); /* Aturan 3 kolom untuk semua ukuran layar */
  gap: var(--sp-fib-4);
}
